...
class Database {
    ...
    private function connect($dbUserName, $whichPass, $dbName) {
        ...
        $dsn = 'mysql:host=smeja.pdx1-mysql-a7-6a.dreamhost.com;port=3306;dbname=';

        if (self::DB_DEBUG) {
            print '<p>Username: ' . $dbUserName;
            print '<p>DSN: ' . $dsn . $dbName;
            print '<p>PW: ' . $whichPass . ' ' .$dbPass;
        }
    ...
    // many times quotes just won't work becasue PDO will get rid of them.
    // This query will not work:
    // SELECT fldFirstName, fldLastName FROM tblPeople WHERE fldFirstName > 'M'
    // however if you pass the M in the value array this query would work and is
    // how it should be done anyway
    //    SELECT fldFirstName, fldLastName FROM tblPeople WHERE fldFirstName > ?
    // when we use htmlentites its good but if we want < or > in query wll &lt; is
    // not < so our query would not work. We need to replace those back.
    function sanitizeQuery($query, $areSpacesAllowed = false, $areSemiColonAllowed = false, $areQuotesAllowed = false, $lessThanOk = false, $greaterThanOk = false) {
...
// #########################################################################
    public function what($query, $values = '') {

        print '<p>What should it look like.</p>';

        $totalWhereClause = $this->countWhere($query);
        $totalConditions = $this->countConditions($query);
        $totalQuotes = $this->countQuotes($query);
        $totalSymbols = $this->countSymbols($query);
        $totalSemiColons = $this->countSemiColons($query);
        ...
        //hilight potentional problems
        $printQuery = str_replace("Q", "<mark>Q</mark>", $query);
        $printQuery = str_replace("&", "<mark>&</mark>", $printQuery);

        print '<p>Query: ' . $printQuery . PHP_EOL;

        $tail = '';
        if ($totalSemiColons != 0) {
            $tail = ", " . $totalWhereClause . ", " . $totalConditions . ", " . $totalQuotes . ", " . $totalSymbols . ", " . $totalSemiColons;
        } else if ($totalSymbols != 0) {
            $tail = ", " . $totalWhereClause . ", " . $totalConditions . ", " . $totalQuotes . ", " . $totalSymbols;
        } else if ($totalQuotes != 0) {
            $tail = ", " . $totalWhereClause . ", " . $totalConditions . ", " . $totalQuotes;
        } else if ($totalConditions != 0) {
            $tail = ", " . $totalWhereClause . ", " . $totalConditions;
        } else if ($totalWhereClause != 1) {
            $tail = ", " . $totalWhereClause;
        }

        print '<p>if ($thisDatabaseReader->querySecurityOk($query' . $tail . " )) {" . PHP_EOL;
        print '<p>$query = $thisDatabaseReader->sanitizeQuery($query);' . PHP_EOL;
        print '<p>$records = $thisDatabaseReader->select($query, $values);' . PHP_EOL;
        print "<p>}" . PHP_EOL;
